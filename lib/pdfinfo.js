...
function pdfinfo (filename, options) {
  ...
  pdfinfo.prototype.getSync = function() {
    var self = this;
    var child = shell.exec('pdfinfo ' + self.options.additional.join(' '), {async:false, silent:true});
    if (child.code === 0) {
  ...
  pdfinfo.prototype.get = function() {
    var self = this;
    var child = shell.exec('pdfinfo ' + self.options.additional.join(' '), {async:true, silent:true});
